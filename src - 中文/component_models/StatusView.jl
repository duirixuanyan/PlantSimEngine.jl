...
`Status` 结构体的等价物，但使用视图（view）代替 Ref。允许使用与 Status 相同的语法，但通过其它数据结构中的值进行初始化，这样我们希望在数据发生更改时同步更新。

与 `Status` 类似，`StatusView` 在模拟过程中用于存储变量值，主要作为 `TimeStepTable` 的 `TimeStepRow`（详见 [`PlantMeteo.jl` 文档](https://palmstudio.github.io/PlantMeteo.jl/stable/)）和 [`ModelList`](@ref) 中用于存放变量的结构体。
